...
export default function ConnectButton() {
  ...
  const { handleConnect, handleSwitchNetwork, isPending, isWrongNetwork } = useConnectWithNetwork(toast);
  ...
  if (isConnected && isWrongNetwork) return (
    <button onClick={handleSwitchNetwork} disabled={isPending}
      style={{ ...base, background: "#8a5a00", color: "#fff", padding: "8px 16px",
        opacity: isPending ? 0.7 : 1, boxShadow: "4px 4px 10px #c4b0d0, -2px -2px 8px #ffffff" }}
      onMouseEnter={e => e.currentTarget.style.transform = "scale(0.97)"}
      onMouseLeave={e => e.currentTarget.style.transform = "scale(1)"}
    >{isPending ? "Switching…" : "⚠ Switch to Monad"}</button>
  );
  ...
  return (
    <button onClick={handleConnect} disabled={isPending}
      style={{ ...base, background: C.accent, color: "#fff", padding: "8px 20px",
        opacity: isPending ? 0.6 : 1, boxShadow: "4px 4px 10px #3a1848, -2px -2px 8px #7a4490" }}
      onMouseEnter={e => { if (!isPending) e.currentTarget.style.transform = "scale(0.97)"; }}
      onMouseLeave={e => e.currentTarget.style.transform = "scale(1)"}
